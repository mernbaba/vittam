...
export default function SanctionsPanel({ sanctionsData }: { sanctionsData: any[] }) {
    ...
    const stats = useMemo(() => {
        ...
        const avgRate = total > 0 ? sanctionsData.reduce((acc, s) => acc + (s.interest_rate || 0), 0) / total : 0;
    ...
    return (
        <div className="min-h-screen p-6 bg-[#FDF6EE]">
            {/* Header */}
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-teal-700/60 mt-1">Monitor and manage loan sanction letters</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8">
                {/* LEFT LIST */}
                <aside className="bg-white rounded-[2rem] border border-teal-100 p-6 shadow-sm overflow-hidden flex flex-col h-[70vh]">
                    ...
                    <div className="space-y-3 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                        {filteredSanctions.map((s, i) => (
                            <button
                                ...
                                className={`w-full text-left rounded-2xl p-4 border-2 transition-all duration-200 group ${activeSanction?._id === s._id
                                    ? "bg-teal-600 border-teal-600 text-white shadow-lg shadow-teal-200 scale-[1.02]"
                                    : "bg-white border-transparent hover:border-teal-100 hover:bg-teal-50/50"
                                    }`}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <p className={`font-bold text-base ${activeSanction?._id === s._id ? "text-white" : "text-teal-900"}`}>
                                        {s.customer_name}
                                    </p>
                                    <div className={`rounded-full p-1 ${activeSanction?._id === s._id ? "bg-white/20" : "bg-teal-50 text-teal-600"}`}>
                                        {getStatusIcon(s.status)}
                                    </div>
                                </div>
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className={`text-xs uppercase tracking-widest font-bold ${activeSanction?._id === s._id ? "text-teal-100" : "text-teal-500"}`}>
                                            Amount
                                        </p>
                                        <p className={`font-bold ${activeSanction?._id === s._id ? "text-white" : "text-teal-800"}`}>
                                            ₹{(s.loan_amount || 0).toLocaleString()}
                                        </p>
                                    </div>
                                    <p className={`text-xs font-semibold ${activeSanction?._id === s._id ? "text-teal-50" : "text-teal-600/60"}`}>
                ...
                {activeSanction ? (
                            ...
                            <div className="relative z-10">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <Button
                                            asChild
                                            className="rounded-2xl py-3 px-6 h-auto bg-teal-600 hover:bg-teal-700 shadow-lg shadow-teal-200 font-bold text-sm"
                                        >
                                            <Link href={`/letters/${activeSanction._id}`} target="_blank">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-teal-50">
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="pt-3 border-t border-dashed border-teal-100">
                                                <BreakdownRow label="Total Payable" value={activeSanction.total_amount} isTotal />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-teal-50/50 rounded-3xl p-6 border border-teal-50">
                                            <div className="space-y-4">
                                                <BankInfo label="Acc. Holder" value={activeSanction.bank_details?.account_holder_name} />
                                                <BankInfo label="Bank Name" value={activeSanction.bank_details?.bank_name || 'N/A'} />
                                                <BankInfo label="Account #" value={activeSanction.bank_details?.account_number} />
                                                <BankInfo label="IFSC Code" value={activeSanction.bank_details?.ifsc_code} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center p-20 bg-white/50 rounded-[3rem] border-2 border-dashed border-teal-100 h-[70vh]">
                        ...
                        <p className="text-teal-600 mt-2">Choose a record from the list to view detailed analytics</p>
                    </div>
                )}
            </div>
        </div>
...
function StatCard({ icon, label, value, bgColor }: { icon: any; label: string; value: string; bgColor: string }) {
    return (
        <div className={`p-6 rounded-[2rem] border border-white shadow-sm ${bgColor} flex items-center gap-5 transition-transform hover:scale-[1.03] duration-300`}>
...
function DetailBox({ label, value, subValue }: { label: string; value: string; subValue: string }) {
    return (
        <div className="bg-teal-50/50 rounded-[1.5rem] p-5 border border-teal-50 group hover:bg-teal-600 transition-all duration-300">
            <p className="text-xs font-bold text-teal-600 mb-1 group-hover:text-white/80 transition-colors uppercase tracking-wider">{label}</p>
            <p className="text-2xl font-black text-teal-950 group-hover:text-white transition-colors">{value}</p>
            <p className="text-xs font-bold text-teal-700/60 mt-2 group-hover:text-white/60 transition-colors italic">{subValue}</p>
        </div>
    );
}

function BreakdownRow({ label, value, isTotal = false }: { label: string; value: number; isTotal?: boolean }) {
    return (
        <div className="flex justify-between items-center py-1">
            <p className={`text-sm ${isTotal ? "font-black text-teal-950" : "font-semibold text-teal-700/70"}`}>{label}</p>
            <p className={`text-sm ${isTotal ? "text-xl font-black text-teal-600" : "font-bold text-teal-900"}`}>
...
function BankInfo({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex justify-between items-center">
            <p className="text-sm font-bold text-teal-800/40 uppercase tracking-widest text-[10px]">{label}</p>
